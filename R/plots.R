...
.plotly_aurora <- function(p, legend_pos = "h", hide_legend = FALSE) {
  p |>
    plotly::layout(
      ...
      font          = list(family = "Inter", color = silvermoon$ink_soft,
                           size = 12),
      margin        = list(l = 50, r = 20, t = 50, b = 40),
      hoverlabel    = list(bgcolor = silvermoon$surface,
                           bordercolor = silvermoon$champagne,
                           font = list(family = "JetBrains Mono",
                                       color = silvermoon$ink)),
      xaxis = list(gridcolor = silvermoon$hairline,
                   zerolinecolor = silvermoon$hairline,
                   tickcolor = silvermoon$muted),
      yaxis = list(gridcolor = silvermoon$hairline,
                   zerolinecolor = silvermoon$hairline,
                   tickcolor = silvermoon$muted),
      legend = list(orientation = if (legend_pos == "h") "h" else "v",
                    bgcolor = "rgba(0,0,0,0)",
                    font = list(color = silvermoon$ink_soft)),
    ...
    plotly::layout(
      title = list(text = "<b>Evolución de precios</b>",
                   font = list(family = "Crimson Pro",
                               color = silvermoon$bone, size = 18),
                   x = 0.02),
      yaxis = list(title = "USD", type = "log",
                   gridcolor = silvermoon$hairline),
      xaxis = list(title = "")
    )
}
  ...
  .plotly_aurora(p) |>
    plotly::layout(
      ...
      shapes = list(list(type = "line", x0 = min(df$date), x1 = max(df$date),
                         y0 = 100, y1 = 100,
                         line = list(color = silvermoon$muted,
                                     dash = "dot", width = 1)))
    )
}
...
plot_candlestick <- function(df, sma = c(20, 50)) {
  ...
  p_price <- plotly::plot_ly(df) |>
    plotly::add_trace(
      ...
      increasing = list(line = list(color = silvermoon$mint),
                        fillcolor = silvermoon$mint),
      decreasing = list(line = list(color = silvermoon$crimson),
                        fillcolor = silvermoon$crimson),
      name = sym
    ) |>
    plotly::layout(xaxis = list(rangeslider = list(visible = FALSE)))

  if (nrow(smas) > 0) {
    sma_colors <- c(silvermoon$champagne, silvermoon$electric,
                    silvermoon$violet)[seq_along(sma)]
  ...
  p_vol <- plotly::plot_ly(df) |>
    plotly::add_bars(
      ...
      marker = list(color = ifelse(df$close >= df$open,
                                   paste0(silvermoon$mint, "55"),
                                   paste0(silvermoon$crimson, "55"))),
...
plot_rsi <- function(df, period = 14) {
  ...
  p <- plotly::plot_ly(d, x = ~date, y = ~rsi,
                       type = "scatter", mode = "lines",
                       line = list(color = silvermoon$champagne, width = 1.6),
                       fill = "tozeroy",
                       fillcolor = paste0(silvermoon$champagne, "22"),
                       name = paste0("RSI(", period, ")"))
  .plotly_aurora(p) |>
    plotly::layout(
      title  = list(text = paste0("<b>RSI(", period, ")</b>"),
                    font = list(family = "Crimson Pro",
                                color = silvermoon$bone, size = 16),
                    x = 0.02),
      ...
      shapes = list(
        list(type = "line", y0 = 70, y1 = 70, x0 = min(d$date),
             x1 = max(d$date),
             line = list(color = silvermoon$crimson, dash = "dot")),
        list(type = "line", y0 = 30, y1 = 30, x0 = min(d$date),
             x1 = max(d$date),
             line = list(color = silvermoon$mint, dash = "dot"))
      )
    )
}
...
plot_corr_heatmap <- function(df) {
  ...
  p <- plotly::plot_ly(
    ...
    colorscale = list(
      c(0,    silvermoon$crimson),
      c(0.5,  silvermoon$surface_alt),
      c(1,    silvermoon$mint)
    ),
    zmin = -1, zmax = 1,
    text = round(cmat, 2),
    hovertemplate = "%{x} ~ %{y}<br>r = %{z:.2f}<extra></extra>",
    colorbar = list(title = "ρ", tickfont = list(color = silvermoon$ink_soft))
  )
  .plotly_aurora(p) |>
    plotly::layout(
...
plot_market_treemap <- function(snapshot) {
  d <- snapshot |>
    ...
    dplyr::mutate(
      ...
      color = ifelse(is.na(change_24h), silvermoon$muted,
                     ifelse(change_24h >= 0, silvermoon$mint,
                            silvermoon$crimson))
    )
  ...
  p <- plotly::plot_ly(
    ...
    marker = list(colors = ~color,
                  line = list(color = silvermoon$bg, width = 2))
  )
...
plot_calendar_heatmap <- function(df, sym = "BTC") {
  ...
  ggp <- ggplot2::ggplot(d,
    ...
    ggplot2::geom_tile(colour = silvermoon$bg, linewidth = 0.4) +
    ggplot2::scale_fill_gradient2(
      low = silvermoon$crimson, mid = silvermoon$surface_alt,
      high = silvermoon$mint, midpoint = 0, name = "% diario"
    ) +
...
plot_forecast_fan <- function(history, fit_obj, symbol = "BTC") {
  ...
  p <- plotly::add_ribbons(
    ...
    line = list(color = "transparent"),
    fillcolor = paste0(silvermoon$champagne, "44"),
  ...
  p <- plotly::add_trace(
    ...
    type = "scatter", mode = "lines",
    line = list(color = silvermoon$ink_soft, width = 1.8),
    name = "Histórico"
  )
  # fit en sample
  p <- plotly::add_trace(
    ...
    type = "scatter", mode = "lines",
    line = list(color = silvermoon$electric, width = 1.2, dash = "dot"),
    name = "Ajuste"
  )
  # forecast media
  p <- plotly::add_trace(
    ...
    type = "scatter", mode = "lines",
    line = list(color = silvermoon$champagne, width = 2.4),
    name = "Pronóstico"
  )
  .plotly_aurora(p) |>
    plotly::layout(
